...
#define	ONLY_TEMP	((1<<TEMP)|(1<<TEMPO))
   ...
   run (sql_safe_query_store_free
        (&sql,
         sql_printf ("SELECT * FROM `%#S` WHERE `tag`=%#s AND `utc`>=%#U AND `utc`<=%#U ORDER BY `utc`", sqltable, tag, stime,
                     etime)));
   if (sqlweather && weathertag)
      run (sql_safe_query_store_free
           (&sql,
            sql_printf ("SELECT * FROM `%#S` WHERE `tag`=%#s AND `utc`>=%#U AND `utc`<=%#U ORDER BY `utc`", sqlweather, weathertag,
                        stime, etime)));
   sql_close (&sql);
   for (d = 0; d < MAX; d++)
      ...
      for (d = 0; d < MAX; d++)
         if (d != TEMPO || !data[TEMP].count)
            if (data[d].count)
            {
               ...
               xml_addf (t, "@transform", "translate(%d,%d)scale(1,-1)", x * 40 + 40,
                         (int) ((data[d].max - 0.1) * data[d].scale - 10));

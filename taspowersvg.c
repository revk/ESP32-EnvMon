...
FNV32_progressive (unsigned int hash, const unsigned char *data, int count)
{
...
static inline unsigned int
FNV32_bitfold (unsigned int hash, int bits)
{
   if (bits >= 32)
...
static inline unsigned int
FNV32 (const unsigned char *data, int count, int bits)
{
   return FNV32_bitfold (FNV32_progressive (FNV32_OFFSET_BASIS, data, count), bits);
}

void
colour (xml_t g, const char *tag, const char *name)
{
...
main (int argc, const char *argv[])
{
   ...
   else
   {
      ...
      for (int x = 0; x <= periods; x++)
      {
         ...
         xml_addf (l, "@d", "M%d,%dL%d,%d", left + x * xsize, top - bottom - base * ysize * 1000 / ydiv, left + x * xsize,
                   top - bottom - (kwh - base) * ysize * 1000 / ydiv);
      }
      for (int y = base; y <= kwh; y += ystep)
      {
         xml_t l = xml_element_add (grid, "path");
         xml_addf (l, "@d", "M%d,%dL%d,%d", left, top - bottom - y * ysize * 1000 / ydiv, left + periods * xsize,
                   top - bottom - y * ysize * 1000 / ydiv);
      }
   }
   int y = 0;
   if (title)
